async function fetchPrem() {
    ...
    const res = await fetch('https://sports-live-scores.p.rapidapi.com/football/rankings/1', options)
    const record = await res.json()
    document.getElementById("premrank").innerHTML = record.rankings.map(item => 
        {
            if (item.Rank < 5) {
