async function fetchLiga() {
    ...
    const res = await fetch('https://sports-live-scores.p.rapidapi.com/football/rankings/36', options)
    const record = await res.json()
    document.getElementById("ligarank").innerHTML = record.rankings.map(item => 
        {
            if (item.Rank < 5) {
